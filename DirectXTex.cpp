...
// PNG operations
HRESULT GetMetadataFromPNGFile(const DXTexWChar *szfile, DirectX::TexMetadata &metadata) noexcept
{
	return DirectX::GetMetadataFromPNGFile(convertString(szfile).c_str(), metadata);
}

// JPEG operations
HRESULT GetMetadataFromJPEGFile(const DXTexWChar *szfile, DirectX::TexMetadata &metadata) noexcept
{
	return DirectX::GetMetadataFromJPEGFile(convertString(szfile).c_str(), metadata);
}

// EXR operations
HRESULT GetMetadataFromEXRFile(const DXTexWChar *szfile, DirectX::TexMetadata &metadata) noexcept
{
	return DirectX::GetMetadataFromEXRFile(convertString(szfile).c_str(), metadata);
}
...
// PNG operations
HRESULT LoadFromPNGFile(const DXTexWChar *szfile, DirectX::TexMetadata *metadata, ScratchImageT *image) noexcept
{
	return DirectX::LoadFromPNGFile(convertString(szfile).c_str(), metadata, *image->image);
}

HRESULT SaveToPNGFile(const DirectX::Image &image, const DXTexWChar *szfile) noexcept
{
	return DirectX::SaveToPNGFile(image, convertString(szfile).c_str());
}

// JPEG operations
HRESULT LoadFromJPEGFile(const DXTexWChar *szfile, DirectX::TexMetadata *metadata, ScratchImageT *image) noexcept
{
	return DirectX::LoadFromJPEGFile(convertString(szfile).c_str(), metadata, *image->image);
}

HRESULT SaveToJPEGFile(const DirectX::Image &image, const DXTexWChar *szfile) noexcept
{
	return DirectX::SaveToJPEGFile(image, convertString(szfile).c_str());
}

// EXR operations
HRESULT LoadFromEXRFile(const DXTexWChar *szfile, DirectX::TexMetadata *metadata, ScratchImageT *image) noexcept
{
	return DirectX::LoadFromEXRFile(convertString(szfile).c_str(), metadata, *image->image);
}

HRESULT SaveToEXRFile(const DirectX::Image &image, const DXTexWChar *szfile) noexcept
{
	return DirectX::SaveToEXRFile(image, convertString(szfile).c_str());
}
