...
DirectX::ScratchImage* CreateScratchImage()
{
...
HRESULT SaveToWICMemory(_In_ const DirectX::Image& image, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _Out_ DirectX::Blob& blob, _In_opt_ const GUID* targetFormat, SetCustomProps customProps)
{
	return DirectX::SaveToWICMemory(image, flags, guidContainerFormat, blob, targetFormat, customProps);
}
HRESULT SaveToWICMemory2(_In_count_(nimages) const DirectX::Image* images, _In_ size_t nimages, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _Out_ DirectX::Blob& blob, _In_opt_ const GUID* targetFormat, SetCustomProps customProps)
{
	return DirectX::SaveToWICMemory(images, nimages, flags, guidContainerFormat, blob, targetFormat, customProps);
}

HRESULT SaveToWICFile(_In_ const DirectX::Image& image, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _In_z_ const wchar_t* szFile, _In_opt_ const GUID* targetFormat, SetCustomProps customProps)
{
	return DirectX::SaveToWICFile(image, flags, guidContainerFormat, szFile, targetFormat, customProps);
}
HRESULT SaveToWICFile2(_In_count_(nimages) const DirectX::Image* images, _In_ size_t nimages, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _In_z_ const wchar_t* szFile, _In_opt_ const GUID* targetFormat, SetCustomProps customProps)
{
...
HRESULT GenerateMipMaps3D2(_In_reads_(nimages) const DirectX::Image* srcImages, _In_ size_t nimages, _In_ DirectX::TEX_FILTER_FLAGS filter, _In_ size_t levels, _Out_ DirectX::ScratchImage& mipChain)
{
...
HRESULT CreateTextureEx2(_In_ ID3D11Device* pDevice, _In_ DirectX::ScratchImage* img, _In_ uint32_t usage, _In_ uint32_t bindFlags, _In_ uint32_t cpuAccessFlags, _In_ uint32_t miscFlags, _In_ DirectX::CREATETEX_FLAGS createFlags, _Outptr_ ID3D11Resource** ppResource) noexcept
{
	return DirectX::CreateTextureEx(pDevice, img->GetImages(), img->GetImageCount(), img->GetMetadata(), (D3D11_USAGE)usage, bindFlags, cpuAccessFlags, miscFlags, createFlags, ppResource);
}
