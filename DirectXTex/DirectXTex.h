...
#pragma region ScratchImage Methods
	...
	API DirectX::ScratchImage* CreateScratchImage();
...
#pragma region ImageIO
	...
	API HRESULT SaveToWICMemory(_In_ const DirectX::Image& image, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _Out_ DirectX::Blob& blob, _In_opt_ const GUID* targetFormat = nullptr, SetCustomProps customProps = nullptr);
	API HRESULT SaveToWICMemory2(_In_count_(nimages) const DirectX::Image* images, _In_ size_t nimages, _In_ DirectX::WIC_FLAGS flags, _In_ GUID guidContainerFormat, _Out_ DirectX::Blob& blob, _In_opt_ const GUID* targetFormat = nullptr, SetCustomProps customProps = nullptr);
...
#pragma region Texture conversion, resizing, mipmap generation, and block compression
	...
	API HRESULT GenerateMipMaps3D2(_In_reads_(nimages) const DirectX::Image* srcImages, _In_ size_t nimages, _In_ DirectX::TEX_FILTER_FLAGS filter, _In_ size_t levels, _Out_ DirectX::ScratchImage& mipChain);
	// levels of '0' indicates a full mipchain, otherwise is generates that number of total levels (including the source base image)
	// Defaults to Fant filtering which is equivalent to a box filter
...
#pragma region Direct3D 11 functions
	...
	API HRESULT CreateTextureEx2(_In_ ID3D11Device* pDevice, _In_ DirectX::ScratchImage* img, _In_ uint32_t usage, _In_ uint32_t bindFlags, _In_ uint32_t cpuAccessFlags, _In_ uint32_t miscFlags, _In_ DirectX::CREATETEX_FLAGS createFlags, _Outptr_ ID3D11Resource** ppResource) noexcept;
